...
// Use jsdom for a real DOMParser if available, otherwise minimal stub.
try {
  const { JSDOM } = require('jsdom');
  global.DOMParser = class {
    parseFromString(html, type) { return new JSDOM(html).window.document; }
  };
} catch (_) {
  global.DOMParser = class {
    parseFromString() { return { body: { childNodes: [] }, documentElement: { outerHTML: '', childNodes: [] } }; }
  };
}
...
(function () {
  ...
  // XSS via innerHTML in string that looks like safe code.
  checkProject('innerHTML hidden in string concat',
    {
      'x.html': '<html><body><script src="x.js"></script></body></html>',
      'x.js': 'var safe = "totally safe";\ndocument.body.innerHTML = "<img src=x onerror=alert(1)>";'
    },
    ['x.js'],
    { 'x.js': c => /createElement/.test(c) && !/innerHTML/.test(c) && !/onerror/.test(c) }
  );

  // Inline event with encoded characters trying to break out.
  checkProject('encoded onclick payload',
    {
  ...
  // Style with expression() / url(javascript:) injection.
  checkProject('CSS injection via style attr',
    {
    ...
    ['p.html', 'p.handlers.js'],
    {
      ...
      'p.handlers.js': c => /setProperty/.test(c) && !/style=/.test(c)
    }
  );

  // javascript: URL with encoding.
  checkProject('javascript: URL with encoding',
    {
  ...
  // innerHTML inside eval string — the tokenizer correctly sees this
  // as a string argument to eval, not an actual innerHTML assignment.
  checkProject('innerHTML in eval not converted',
    {
      'p.html': '<html><body><script src="p.js"></script></body></html>',
      'p.js': 'eval("document.body.innerHTML = \'<b>xss</b>\'");'
    },
    [],
    {}
  );

  // Variable named innerHTML — should not trigger conversion.
  checkProject('variable named innerHTML',
    {
      'p.html': '<html><body><script src="p.js"></script></body></html>',
  ...
  // Multiple innerHTML on same element.
  checkProject('double innerHTML same element',
    {
      'p.html': '<html><body><script src="p.js"></script></body></html>',
  ...
  // Nested quotes in onclick trying to break attribute parsing.
  checkProject('nested quotes in onclick',
    {
    ...
    {
      'p.html': c => !/onclick/.test(c),
      'p.handlers.js': c => /addEventListener/.test(c) && /alert\("hello"\)/.test(c)
    }
  );

  // Script tag inside innerHTML string — createElement('script') is safe
  // (textContent doesn't execute), but verify it's converted.
  checkProject('script in innerHTML string',
    {
      'p.html': '<html><body><script src="p.js"></script></body></html>',
      'p.js': 'el.innerHTML = "<script>alert(1)<\\/script>";'
    },
    ['p.js'],
    { 'p.js': c => /createElement/.test(c) && !/innerHTML/.test(c) }
  );

  // data: URL in href — not a javascript: URL, should not be extracted.
  // But the <script> inside the data URL must not be treated as an inline script.
  checkProject('data: URL in href',
    {
      'p.html': '<html><body><a href="data:text/html,test">x</a></body></html>'
    },
    [],
    {}
  );

  // Prototype pollution attempt via __proto__ in concat.
  ...
  // Empty innerHTML — should still convert (clear children).
  ...
  // innerHTML += with no prior content.
  ...
  // Fake innerHTML in a comment — should be ignored.
  checkProject('innerHTML in comment ignored',
    {
      'p.html': '<html><body><script src="p.js"></script></body></html>',
  ...
  // onclick with semicolons and multiple statements.
